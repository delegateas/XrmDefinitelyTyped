...
let interpretCrmData (gSettings: XdtGenerationSettings) out formsToIntersect viewsToIntersect (rawState: RawState) labelMapping =
  ...
  let entityMetadata =
    rawState.metadata |> Array.Parallel.map (interpretEntity schemaNames rawState.nameMap labelMapping (gSettings.deprecatedPrefix))
